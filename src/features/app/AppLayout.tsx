            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <div className="flex-1 relative flex flex-col min-w-0 bg-white/5 rounded-xl border border-white/10 shadow-2xl overflow-hidden backdrop-blur-md mr-2">
                    <div className="flex-1 flex flex-col mx-auto w-full relative overflow-hidden max-w-lvw">
                        ...
                        {activeView === 'clips' && (
                            <div className="flex flex-col h-full p-6 overflow-hidden">
                                ...
                                <div className="flex-1 flex gap-6 min-h-0 overflow-auto">
                                    ...
                                    {/* LEFT PANEL: History List (Visible ONLY when searching) */}
                                    {searchQuery && (
                                    ...
                                    {/* RIGHT PANEL: Preview & Actions (Always Visible) */}
                                    {/* If searching, this is the 'Detail' view. If not searching, this is the 'Main' view. */}
                                    <div className={`flex flex-col gap-6 overflow-hidden ${searchQuery ? 'w-1/2 shrink-0' : 'w-full max-w-4xl mx-auto'}`}>

                                        {/* Determine what to show: PreviewClip (from search hover) OR ActiveClip (top of list) */}
                                        {/* We use a variable to help TS narration */}
                                        {(() => {
                        ...
                        {activeView === 'plugins' && (
                            <div className="w-full flex-1 flex items-center justify-center text-gray-500"><p>Plugins coming soon...</p></div>
                        )}
                    </div>
                </div>
            </div>
