...
export const TransformControls = ({ clipId }: { clipId: string }) => {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      invoke<Transformer[]>('list_transformer_contributions', { clipId }),
    ])
  ...
  return (
    <div className="absolute bottom-3 right-3 z-10 max-w-80 rounded-lg border border-slate-200 bg-white/95 p-2 shadow-lg backdrop-blur dark:border-white/10 dark:bg-slate-900/95">
      <div className="flex flex-wrap gap-1">
        {items.map(item => (
          <button
            className="rounded px-2 py-1 text-[10px] text-gray-600 hover:bg-slate-100 dark:text-gray-300 dark:hover:bg-white/10"
            disabled={busy}
            key={item.id}
            onClick={() => void previewTransform(item)}
          >
            {item.label}
          </button>
        ))}
      ...
      {preview && (
        <div className="mt-2 border-t border-slate-200 pt-2 dark:border-white/10">
          ...
          <div className="mt-2 flex gap-1">
            ...
            <button
              className="rounded border border-slate-300 px-2 py-1 text-[10px] dark:border-slate-600"
              onClick={() => void invoke('save_transform_result', { resultId: preview.resultId })}
            >
              Save
            </button>
          </div>
