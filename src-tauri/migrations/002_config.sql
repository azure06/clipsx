...
INSERT INTO config_device_values (key, value_json, updated_at) VALUES
...
INSERT INTO config_profile_values (key, value_json, updated_at) VALUES
    ('renderer.preferences', '{}', CAST(strftime('%s', 'now') AS INTEGER) * 1000),
    ('search.syntax_mode', '"simple"', CAST(strftime('%s', 'now') AS INTEGER) * 1000),
    ('artifacts.ocr.enabled', 'true', CAST(strftime('%s', 'now') AS INTEGER) * 1000),
    ('search.embedding.provider', 'null', CAST(strftime('%s', 'now') AS INTEGER) * 1000);
