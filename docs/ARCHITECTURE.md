...
OCR provenance version 2 uses platform-owned local runtimes. Windows image decoding and `Windows.Media.Ocr` recognition run on a dedicated bounded WinRT MTA worker and select an installed user-profile OCR language. macOS uses Vision. Linux invokes Tesseract directly without a shell and reports absence or process failure as derived-work diagnostics. Runtime availability is queried from the actual platform engine; it is not a compile-time claim. OCR output remains derived and a producer-version change rebuilds it without mutating canonical images.
