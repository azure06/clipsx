...
Transforms are explicit byte-producing operations. The host validates parameters, caches an exact short-lived result, and uses those exact result bytes for preview, Copy, Paste, and Save as New Clip. Raster outputs may be served from that expiring cache through a no-store, opaque transform-result image source; they are not inserted into canonical or artifact storage unless the user saves the result as a new clip. Saving creates a linked canonical clip with transform provenance; it never overwrites the source. The output boundary is the only clipboard-write owner.
