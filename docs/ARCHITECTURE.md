...
Meaning-search recovery may discard all text embedding spaces, generations,
jobs, pending sidecar-deletion intents, and sidecar files while preserving
canonical clips and the configured provider. A reset validates that provider
first, recreates its immutable space, and enqueues every ready clip. Reindex All
uses this same full reset so missing or inconsistent derived state never
...
Search typing is isolated from the layout and history/preview render trees. A small search controller owns the raw query subscription and a 300 ms idle debounce for requests; input text updates synchronously, and IME composition is not submitted until it ends. Preview rendering and content statistics reuse unchanged models, with the applied theme passed explicitly to themed renderers.
...
The official registry is a signed-byte trust root. ClipsX verifies a detached Ed25519 signature against embedded key IDs before parsing or replacing its cache; overlapping valid signatures permit key rotation. Registry schema v4 pins release archives and light/dark raster catalog icons independently by SHA-256 and carries reviewed, canonically sorted portable boolean/number setting declarations. Installation requires those declarations to match the packaged manifest. Catalog icons are bounded, format-sniffed, fetched only from the official registry repository, cached by hash, and reverified before being exposed as data URLs. Signed revocations bind package ID, version, and archive checksum. A matching installed registry release is quarantined and cannot be installed, updated, or manually recovered. Unsigned local archives never enter this path and require Developer Mode.
...
The history store distinguishes desired query/filter/source state from the still-displayed page using an explicit stale-results state. A query change invalidates older responses immediately, before the debounce expires. Replacement searches retain the previous rows and selected preview with an updating indicator; result interactions and pagination are paused until a current response commits. Failures retain the stale page with retry, while native search-input editing remains available. A successful response atomically replaces the first page, source outcomes, and cursor; only subsequent pages of that request generation append. Selection survives by clip ID when possible and clears for an empty result set. An unchanged selected preview remains mounted during replacement. Native extension detail surfaces are hidden while results are stale because DOM inertness cannot disable a child webview. Already-started operations, including pending note saves, may finish.

Search-status refreshes triggered by indexing and provider events are coalesced to at most one start per 500 ms, with one active refresh and one trailing refresh. Unmounted consumers ignore late status responses. This scheduling affects status display, not indexing or search ranking.
