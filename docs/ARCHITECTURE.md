...
Large semantic data lives in one disposable `search-index/generation-{id}.sqlite` sidecar per generation. A sidecar stores clean snippets, strategy/source provenance, stable clip/chunk ordinals, packed per-clip signed-int8 scan vectors, and normalized float32 rerank vectors. Packing the scan projection per clip keeps ordinary updates local while reducing the candidate scan from one SQLite row per chunk to at most one row per clip. `clips.db` stores embedding-space identity, generation/job lifecycle, the safe relative sidecar path, size, checkpoint checksum, backend ID, encoding, and candidate policy. Pipeline or model changes create a replacement generation. Ordinary clip changes replace only that clip's rows transactionally in the active generation; the coordinator clears the checkpoint checksum before mutation and may refresh it after a bounded durable checkpoint. SQLite WAL recovery, schema identity, and integrity checks protect the live sidecar between checksums, avoiding a full-generation copy for every clipboard capture.
...
Semantic sidecars are derived data, not canonical databases. Factory reset removes `search-index/`; “Delete Meaning Search index” removes sidecars and generation/job state without touching clips or FTS. The selected contiguous-memory int8 algorithm measured 18.4 ms p95 at the conservative 540,000 × 1,024 target on the qualification machine. The production packed-SQLite physical path must independently meet the same gate before release; labelled-corpus recall, filters, recovery, memory, disk, and every installed release target also remain certification gates.
