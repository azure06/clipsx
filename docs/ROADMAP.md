...
| Semantic search                          | A provider-agnostic planner runs mandatory FTS and optional Ollama text-semantic candidates against shared filters, then unions, fuses, and paginates results. Semantic-only recall, structure-aware HTML/Markdown/RTF/JSON/table/code/plain/OCR chunks, contextual embedding prefixes, distinct-source deduplication, hard bounds, generation-safe rebuilds, source controls, recovery, and fallback diagnostics are implemented. | Validate the complete workflow in installed builds, add layout-aware OCR and AST code strategies when their source contracts exist, and use the registered source boundary for future visual search. | M2    |
...
- Validate semantic-only recall, shared filtering, fused pagination, source fallback,
  and persistent source controls in installed builds.
