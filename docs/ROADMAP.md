...
| Semantic search                          | A provider-agnostic planner runs mandatory FTS and optional Ollama text-semantic candidates against shared filters, then unions, fuses, and paginates results. Semantic-only recall, hard-bounded generic chunks, generation-safe rebuilds, source controls, background indexing, recovery, and fallback diagnostics are implemented. | Add format-specific semantic input/chunk strategies, validate the complete workflow in installed builds, and use the registered source boundary for future visual search. | M2    |
