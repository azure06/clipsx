...
| Semantic search                          | A provider-agnostic planner runs mandatory FTS and optional Ollama text-semantic candidates against shared filters, then unions, fuses, and paginates results. Semantic-only recall, source controls, persistent configuration, background indexing, progress, recovery, and fallback diagnostics are implemented. | Validate the complete workflow in installed builds and use the registered source boundary for any future visual search. | M2    |
